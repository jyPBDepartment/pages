				...
				<view @click="jump(item)" class="box box-border p-y-10 f-12 g-flex" v-for="(item, index) in list" :key="index">
					...
					<view class="info g-f-1">
						<view class="title f-14 g-flex">
							...
							<view class="price f-16">￥{{ item.workPrice }}元/亩</view>
						</view>
						<view class="other g-a-c g-flex ">面积:{{ item.area }}亩</view>
						<view class="other g-a-c g-flex g-j-s-b">日期:{{ item.beginDate }}至{{ item.endDate }}</view>
