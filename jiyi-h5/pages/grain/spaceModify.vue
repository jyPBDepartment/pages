		...
		<view class="init">
			...
			<view class="p-x-10  b-t content-box">
				...
				<view class="info  g-flex g-j-s-b">
					...
					<text v-if="farmingMode == '0'">农活方式：整活</text>
					<text v-if="farmingMode == '1'">农活方式：零活</text>
				</view>
				<view class="info f-12 g-flex g-a-c g-j-s-b">
					<text>时间：{{ beginDate }}至{{ endDate }}</text>
					干活天数：{{ days }}
				</view>
				<view class="info g-a-c g-flex g-j-s-b f-12" style="margin-bottom: 16rpx;">
					联系人：{{ contactsUser }}
					<view>联系电话：{{ contactsPhone }}</view>
				</view>
				<view class="info g-a-c g-flex g-j-s-b f-12">地址：{{ address }}</view>
				...
				<view class="btn g-flex" v-if="isDisplay !== 0">
					...
					<view class="g-f-1"><u-button type="success" @click="updateInfo(id)" shape="circle">修改信息</u-button></view>
				</view>
			</view>
		</view>
			...
			uni.request({
				...
				success: res => {
					if (res.data.state == 0) {
						...
						//取消跳转
						uni.navigateTo({
							url: '../catalog/agriculturalMachinery?index=2'
						});
					} else {
						uni.showToast({
