  ...
  >
    ...
    <slot>
      <el-form :rules="rules" ref="editForm" :model="editForm" :label-position="labelPosition" label-width="120px" @submit.native.prevent>
        ...
        <el-button type="success"  @click="handle" style="background-color:rgb(132, 193, 255);border:none;color:white;font-size:12px">添加二级菜单</el-button>
       <el-button type="success"  @click="cancal" style="background-color:rgb(132, 193, 255);border:none;color:white;font-size:12px">收起二级菜单</el-button>
        <!-- <el-form-item label="上级导航" prop="subId" v-if="isShow">
            <el-select v-model="editForm.subId" placeholder="请输入上级导航" style=" width:70%;" id="test" >
              <el-option
...
                :value="item.value">
        ...
        <el-form-item label="下拉英文内容" prop="dropDownEnName" v-if="isShow">
          <el-input type="text" v-model="editForm.dropDownEnName" placeholder="下拉英文内容"  style=" width:70%;" ></el-input>
        </el-form-item>
         ...
         <el-form-item label="导航路径"  prop="path"  v-if="isShow">
          <el-input type="textarea" v-model="editForm.path" placeholder="请输入导航路径"  style=" width:70%;" ></el-input>
        </el-form-item>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        subId: [
          { required: true, message: '请输入上级导航', trigger: 'blur' }
        ],
         dropDownEnName: [
...
cancal(){
  this.isShow=false;
    ...
    findContext: function() {
      ...
      api
        .testAxiosGet(ApiPath.url.findAllNavChildren, params)
        .then(res => {
          if (res.status == "0") {
