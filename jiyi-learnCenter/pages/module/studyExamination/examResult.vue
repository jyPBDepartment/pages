<template>
	<view class="content">
		...
		<view class="contain">
			...
			<view class="examResult">
				<view class="isPass" :style="examGrade >= passScore ? 'background-color:#6ca984;' : 'background-color: #E51C2E;'">
					<image
						:src="
							examGrade >= passScore ? 'http://60.205.246.126/images/2020/10/29/1603956788960268.png' : 'http://60.205.246.126/images/2020/10/29/1603956449935082.png'
						"
						class="examIcon"
					></image>
				...
				<view class="currentGrade">
					<view class="getScore" :style="examGrade >= passScore ? 'color:#6ca984;' : 'color: #E51C2E;'">
						<b>{{ examGrade }}</b>
						分
					</view>
					<view class="full">满分{{ totalScore }}分</view>
				</view>
				<view class="tips">{{ examGrade >= passScore ? '恭喜你通过考试！' : '不要紧，山野都有雾灯，再试一次吧！' }}</view>
			</view>
			<!-- 下划线 -->
			<!-- 	<view class="line">
			...
			<view class="questionAnswer">
				<!-- 答题卡 -->
				<view class="answerCard card-box">
					<view class="answerTitle">答题卡（{{ wrongCount }}/{{ count }}）</view>
					...
					<view class="answerIndex">
						<view class="answer" v-for="(subjectItem, subjectIndex) in examTableList" :key="subjectIndex">
							<view class="answerCorrect" :style="subjectItem.queIsCorrect == 1 ? 'background-color: #27B148;' : 'background-color:#E51C2E;'">
								{{ subjectIndex + 1 }}
							</view>
						</view>
					</view>
			...
			<!-- 错题解析 -->
			<view
				class="analysis card-box animate__animated animate__slideInRight"
				v-for="(titleItem, titleIndex) in examTableList"
				:key="titleIndex"
				...
				<view class="queAnswer" v-for="(item1, index1) in titleItem.queAnalysis" :key="index1">
					<view class="answerStyle">
						<image
							:src="
								item1.queIndex != titleItem.correctAnswers
									? 'http://60.205.246.126/images/2020/10/30/1604029013343857.png'
									: 'http://60.205.246.126/images/2020/10/30/1604029038740080.png'
							"
							class="answerIcon"
						></image>
		...
		<view style="display: flex;">
			<u-button style="flex: 1;" v-if="examGrade >= passScore" type="primary" @click="continueExam()">继续考试</u-button>
		
			<u-button  style="flex: 1;" v-if="examGrade < passScore" type="primary" @click="returnExam()">重新答题</u-button>
		
			<u-button style="flex: 1;"  type="error" @click="exitExam()">退出考试</u-button>
		</view>
	</view>
</template>
				...
				success: res => {
					if (res.data.state == 0) {
						...
						for (let i = 0; i < result.length; i++) {
							if (result[i].queType == '1') {
								//选择题
								radioCount = radioCount + 1;
								if (result[i].queIsCorrect == '2') {
									//如果是错题
