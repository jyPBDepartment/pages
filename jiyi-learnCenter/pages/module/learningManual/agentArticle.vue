<template>
	<view class="content">
		<!-- 头部 -->
		<header-box title="手册内容" @backTo="backTo"></header-box>
		<!-- 	<view class="head">
		...
		<view class="screen-box">
			<u-tabs :list="labelList" :current="current" @change="change" font-size="30" item-width="150" :show-bar="false" active-color="red" class="u-sticky"></u-tabs>
		</view>
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption">
			...
			<view class="list" v-for="(item, index) in agentList" :key="index" @click="agentArticleJump(item.id)">
				<view class="listOne">
					<view class="img"><image class="listImg" :src="item.url"></image></view>
					<view class="listText">{{ item.title }}</view>
				</view>
				<view class="listSen">
					<view class="bride">{{ item.studyNum }}人已学</view>
				</view>
				<view><u-line class="underline"></u-line></view>
			</view>
		</mescroll-body>
	</view>
</template>
