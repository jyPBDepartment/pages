<template>
  <view class="offline-details">
    ...
    <view class="course-title course-title-b" v-show="current == 0">
      <view class="name">主讲人：{{ name }}</view>
      <view class="time">人数限制：{{ stuLimit }} 人</view>
      <view class="time">课程介绍：{{ courseIntroduction }}</view>
    </view>
    <view class="course-title course-title-b" v-show="current == 1">
...
export default {
  ...
  methods: {
        ...
        success: (res) => {
          ...
          if (res.data.code == 200) {
            ...
            setTimeout(() => {
              ...
              uni.reLaunch({
                url:
                  "./offlineDetails?status=" + this.status + "&id=" + this.id,
              });
            }, 500);
          } else {
            uni.showToast({
    ...
    signUp(val) {
      uni.navigateTo({
        url:
          "../offlineCourses/offlineSignUp?index=0&title=" +
          this.title +
          "&status=" +
          this.status +
          "&id=" +
