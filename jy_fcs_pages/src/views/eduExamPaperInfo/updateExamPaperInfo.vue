<template>
  ...
  >
    ...
    <!-- 按钮区 -->
    <span slot="footer">
      <el-button type="primary" @click="showExamPaper('editForm')"
        >预览</el-button
      >
      <el-button
        type="primary"
  ...
  data() {
    return {
      ...
      rules: {
        ...
        totalScore: [
          { required: true, message: "请输入总分数", trigger: "change" },
        ],
        passScore: [
          { required: true, message: "请输入通过分数", trigger: "change" },
        ],
        answerTime: [
          { required: true, message: "请输入答题时间", trigger: "change" },
        ],
      },
    };
  },
