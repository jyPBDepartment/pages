<template>
  ...
  >
    ...
    <!-- 按钮区 -->
    <span slot="footer">
      <el-button type="primary" icon="el-icon-check" @click="savePictureInfo('editForm')" v-loading.fullscreen.lock="fullscreenLoading">保存</el-button>
      ...
      this.$refs[editData].validate((valid) => {
        if (valid) {
          if (this.imgUrl != "") {
            ...
            let params = {
              eduPictureInfoEntity: this.editForm,
            };
            api.testAxiosGet(ApiPath.url.addPictureInfo, params)
              .then((res) => {
                let code = res.state;
                if (code == "0") {
                  this.close();
