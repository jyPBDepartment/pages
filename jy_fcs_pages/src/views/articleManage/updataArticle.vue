<template>
  ...
  >
    ...
    <!-- 按钮区 -->
    <span slot="footer">
      <el-button type="primary" icon="el-icon-check" size="medium" @click="updateArticle('editForm')">保存</el-button>
...
export default {
  ...
  methods: {
    ...
    findSectionName: function () {
      let params = {};
      api.testAxiosGet(ApiPath.url.findListSectionId, params)
        .then((res) => {
          if (res.code == "200") {
      ...
      this.$refs[editData].validate((valid) => {
        ...
        if (valid) {
          if (this.imgUrl != "") {
            ...
            api.testAxiosGet(ApiPath.url.updateArticle, params)
              .then((res) => {
                let code = res.state;
                if (code == "0") {
                  this.close();
