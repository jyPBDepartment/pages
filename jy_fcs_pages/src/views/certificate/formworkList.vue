...
<template>
  <div class="adminFunction">
    ...
    <el-table :data="tableData" border style="width: 100%;" highlight-current-row size="mini">
      ...
      <el-table-column prop="name" label="模板名称" align="center" min-width="20%"  :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="remark" label="模板描述" align="center" min-width="20%"  :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="url" label="预览" align="center" min-width="5%">
        <template slot-scope="scope" style="height:120px">
          <el-image :src="scope.row.url" style="width:33px;height:33px;"  @click="showImg(scope,dialogVisible =true)"></el-image>
        </template>
      </el-table-column>
...
import addLesson from "@/views/certificate/addFormwork";
import editLesson from "@/views/certificate/editFormwork";
...
export default {
  ...
  methods: {
    //图片预览
    showImg: function (scope){
       let params = {
        id: scope.row.id,
      };
      api.testAxiosGet(ApiPath.url.findFormworkById, params)
        .then((res) => {
          let code = res.state;
          if (code == "0") {
    ...
    statusEnable: function (scope) {
      ...
      api
        .testAxiosGet(ApiPath.url.changeFormworkStatus, params)
        .then((res) => {
          let code = res.state;
          if (code == "1") {
    ...
    search: function (parameter) {
      ...
      api
        .testAxiosGet(ApiPath.url.findFormworkPage, params)
        .then((res) => {
          let code = res.state;
          if (code == "0") {
    ...
    deleteCase: function (scope) {
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          api.testAxiosGet(ApiPath.url.deleteFormworkById, params).then((res) => {
            let code = res.state;
